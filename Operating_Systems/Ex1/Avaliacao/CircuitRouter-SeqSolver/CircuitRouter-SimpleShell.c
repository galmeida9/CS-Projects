...
int main(int argc, char *argv[]) {
	int pid, maxChildren = -1, pos = 0, i, j, size = ARRAYSIZE;
	char *buffer, *token, *path, *cpy;
	...
	do { 
		...
		else if (strcmp(cpy, "info") == 0 )
            printf("O numero de Processos lancados ate agr sao: %d\n", pos);
